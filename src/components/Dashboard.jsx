                    ...
                    {filter === 'analytics' ? (
                        ...
                        <div className="main-content-overlay">
                            ...
                            {filter === 'my-day' && (() => {
                                ...
                                const totalAT = filteredTasks.reduce((sum, t) => {
                                    if (!t.actualTime) return sum;
                                    // Tasks completed today: always count their actual time
                                    if (t.status === 'completed' && t.completedAt?.startsWith(today)) return sum + t.actualTime;
                                    // Pending/in-progress tasks: only count if actual time was entered today
                                    if (t.actualTimeDate === today) return sum + t.actualTime;
                                    return sum;
                                }, 0);
